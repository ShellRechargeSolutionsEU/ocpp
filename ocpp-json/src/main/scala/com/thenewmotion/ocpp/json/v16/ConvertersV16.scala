...
object ConvertersV16 {
  def toV15(msg: messages.Message): Message = msg match {
    ...
    case messages.BootNotificationRes(registrationAccepted, currentTime, interval) =>
      BootNotificationRes(registrationAccepted.toStatusString, currentTime, interval.toSeconds.toInt)
    ...
    case messages.StopTransactionReq(transactionId, idTag, timestamp, meterStop, transactionData) =>
      StopTransactionReq(transactionId = transactionId,
                         ...
                         reason = "",
                         meterStop = meterStop,
                         transactionData = Some(transactionData.map(_.toV16)))
    ...
    case messages.MeterValuesReq(scope, transactionId, meters) =>
      MeterValuesReq(scope.toOcpp, transactionId, Some(meters.map(_.toV16)))
  ...
  def fromV15(msg: Message): messages.Message = msg match {
    ...
    case MeterValuesReq(connectorId, transactionId, values) =>
      val meters: List[messages.Meter] = values.fold(List.empty[messages.Meter])(_.map(meterFromV16))
  ...
  private implicit class RichTransactionData(self: messages.TransactionData) {
    def toV16: Meter = Meter(timestamp = ZonedDateTime.now, sampledValue = self.meters.map(_.toV16))
  }
  ...
  private def transactionDataFromV16(v16td: List[Meter]): List[messages.TransactionData] =
    v16td.map(metersFromV16)

  private def metersFromV16(v16mv: Meter): messages.TransactionData =
    messages.TransactionData(v16mv.sampledValue.map(meterFromV16))
